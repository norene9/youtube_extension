...
var blockUrl = chrome.runtime.getURL("pdfviewer/pdf.html") ;
...
if(tab1.url.includes('youtube')){
  ...
  chrome.tabs.getSelected(null, function(tab1){
    if(tab1.url.indexOf('pdf')>-1){
      console.log(cUrl)
      
      chrome.tabs.update(tab1.id, { url: cUrl });
...
function backgroundCheck() {
    chrome.windows.getLastFocused({ populate: true }, function(currentWindow) {
        ...
        tab = null,
        ignoreTick = false;
        for (ind = 0; ind < currentWindow.tabs.length; ind += 1) {
            if (currentWindow.tabs[ind].highlighted) {
              tab = currentWindow.tabs[ind];
            }
          }
        if ( tab) {
            var activeTab = tab;
            if (activeTab !== undefined && activity.isValidPage(activeTab)) {
                var activeUrl = activeTab.title;
